...
namespace CourseLibrary.API.Controllers
{
    ...
    public class AuthorCollectionsController : ControllerBase
    {
        ...
        [ModelBinder(BinderType = typeof(ArrayModelBinder))] IEnumerable<Guid> ids)
        {
            ...
            var authorEntitites = _courseLibraryRepository.GetAuthors(ids);

            if (ids.Count() != authorEntitites.Count())
            {
                return NotFound();
            }

            var authorsToReturn = _mapper.Map<IEnumerable<AuthorDto>>(authorEntitites);

            return Ok(authorEntitites);
        }
